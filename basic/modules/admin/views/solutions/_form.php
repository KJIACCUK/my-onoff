...
/*<?= $form->field($model, 'sections_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\modules\admin\models\Sections::find()->where(['parent_id'=>'3'])->all(),
        'id', 'name'),['prompt' => '']) ?>*/
?>
...
<div class="post-form p-l-20 p-r-20 p-t-20 p-b-20">
    ...
    <?= $form->field($model, 'sections_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\modules\admin\models\Sections::find()
        ...
        ->where(['x.parent_id'=>'3'])->asArray()->all(),
        'id', 'section'), ['prompt' => '']) ?>

    <?php if(Yii::$app->request->url == '/admin/solutions/create'):?>
